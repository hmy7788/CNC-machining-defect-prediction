...
CNC 가공 결함/공구 마모 예측 프로젝트. 아직 분석/모델링 코드는 없고(`src/`, `notebooks/`, `models/`,
`reports/`는 `.gitkeep`만 존재), 대신 에이전트가 안전하게 작업할 수 있도록 하네스(harness)부터 구축된
상태다. 첫 실제 코드를 추가할 때는 아래 "디렉토리 구조" 절의 관례를 따르면 된다.
...
- `.gitattributes`로 텍스트 파일 줄바꿈을 LF로 고정했다 — Windows의 `core.autocrlf=true` 환경에서
  데이터 CSV 전체가 매번 "수정됨"으로 뜨는 문제를 막기 위함.
...
- `scripts/check_structure.py`: `src/`, `tests/`, `scripts/`, `notebooks/`에 금지 패턴(`temp_*` 등)
  파일이 있는지 확인한다.
- `.github/workflows/housekeeping.yml`이 매주 월요일 오전 9시(UTC) + 수동 실행으로 위 두 스크립트와
